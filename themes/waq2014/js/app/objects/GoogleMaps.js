...
(function() {
  window.CustomGmap = (function() {
      ...
      markers: [/*
        {
          ...
          shadow: {
            src: template_url+"/img/germain-dominion-shadow.png",
            width: 68,
            height: 55
          },
    ...
    CustomGmap.prototype.addMarker = function() {
      var iconHeight, iconWidth, iconmid, key, marker, markerCoord, opts, shadowHeight, shadowWidth, shadowmid, _ref, _results,
        _this = this;
      this.marker = [];
      _ref = this.settings.markers;
      _results = [];
      ...
      for (var i = 0; i < length; i++) {
        ...
        opts = {
          ...
          image: marker.image,
          alwaysOpen: marker.isWaq != null
        };
        this.marker[key]["infoWindow"] = new CustomInfoWindow(this.map, opts);
        if (marker.icon != null) {
          iconWidth = marker.icon.width;
          iconHeight = marker.icon.height;
          iconmid = [iconWidth / 2, iconHeight / 2];
          this.marker[key]["icon"] = new google.maps.MarkerImage(marker.icon.src, null, null, new google.maps.Point(iconmid[0], iconmid[1]), new google.maps.Size(iconWidth, iconHeight));
          if (marker.shadow != null) {
            ...
            this.marker[key]["shadow"] = new google.maps.MarkerImage(marker.shadow.src, null, null, new google.maps.Point(shadowmid[0], shadowmid[1]), new google.maps.Size(shadowWidth, shadowHeight));
          }
          this.marker[key]["marker"] = new google.maps.Marker({
            position: markerCoord,
            map: this.map,
            icon: this.marker[key]["icon"],
            shadow: this.marker[key]["shadow"] != null ? this.marker[key]["shadow"] : false,
            visible: true,
            draggable: false,
            cursor: "pointer"
          });
          _results.push(google.maps.event.addListener(this.marker[key]["marker"], 'click', function(e) {
            if (_this.currentOpenedInfoWindow != null) {
              _this.currentOpenedInfoWindow.close();
            }
            _this.marker[key]["infoWindow"].open();
            return _this.currentOpenedInfoWindow = _this.marker[key]["infoWindow"];
          }));
        } else {
  ...
  */
  ...
  window.CustomInfoWindow = (function() {
    function CustomInfoWindow(map, opts) {
      var closeBtn, isMobile, wrap,
        _this = this;
      this.position = opts.coord;
      this.alwaysOpen = opts.alwaysOpen;
      this.map = map;
      closeBtn = !this.alwaysOpen ? "<span class=\"closeBtn\">×</span>" : "";
      isMobile = $('body').hasClass('mobile') ? true : false;
      if (!isMobile) {
      ...
      wrap += "      " + closeBtn + "      <div class=\"padding\">        <span class=\"address\">          " + opts.content + "        </span>";
      if (!isMobile) {
      ...
      this.isVisible = true;
      this.wrap.find('.closeBtn').on('click', function() {
    ...
    CustomInfoWindow.prototype.close = function() {
      return this.wrap.hide();
    };

    CustomInfoWindow.prototype.draw = function() {
      var overlayProjection, pos, wrapImg, wrapImgHeight;
      overlayProjection = this.getProjection();
      pos = overlayProjection.fromLatLngToDivPixel(this.position);
      this.oX = pos.x - this.wrap.outerWidth() / 2;
      this.oY = pos.y - this.wrap.outerHeight() - 30;
      wrapImg = this.wrap.find('img');
      wrapImgHeight = wrapImg.height();
      this.wrap.find('img').css({
        'top': '50%',
      ...
      return this.wrap.css({
        ...
        top: this.oY,
        opacity: 1,
        display: this.alwaysOpen ? 'block' : 'none'
      });
    };
