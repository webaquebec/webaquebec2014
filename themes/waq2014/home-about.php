  ...
  if(!empty($organizers)){
    ...
    foreach($organizers as $organizer){
      $organizers_output .= '<li itemscope itemtype="http://schema.org/Person">';
          ...
          $organizers_output .= '<div class="overlay social-medias reversed">';
              if($fb_url = get_field('facebook_link',$organizer->ID))
                $organizers_output .= '<a href="'.$fb_url.'" class="Facebook"><img src="'.get_bloginfo('template_directory').'/img/icon-social-white-facebook.png" alt="Facebook"></a>';
              if($tw_url = get_field('twitter_link',$organizer->ID))
                $organizers_output .= '<a href="'.$tw_url.'" class="Twitter"><img src="'.get_bloginfo('template_directory').'/img/icon-social-white-twitter.png" alt="Twitter"></a>';
              if($ln_url = get_field('linkedin_link',$organizer->ID))
                $organizers_output .= '<a href="'.$ln_url.'" class="LinkedIn"><img src="'.get_bloginfo('template_directory').'/img/icon-social-white-linkedin.png" alt="LinkedIn"></a>';
              if($gp_url = get_field('google_link',$organizer->ID))
                $organizers_output .= '<a href="'.$gp_url.'" class="Google+"><img src="'.get_bloginfo('template_directory').'/img/icon-social-white-googleplus.png" alt="Google+"></a>';
          $organizers_output .= '</div>';
